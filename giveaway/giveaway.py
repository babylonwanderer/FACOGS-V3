...
log = logging.getLogger("red.giveaway")

__author__ = 'BabylonWanderer'
__version__ = '4.2.3'
...
class Giveaway(BaseCog):
    """Run simple Giveaway for your server."""
    ...
    async def start(self, ctx, timer, *, title: str):
       ...
       # embed.add_field(name="Winners", value=f'{winners}')
        embed.add_field(name="Giveaway ID#", value=f'{msg.id}')
        role_info = f'{", ".join(str_roles) if roles else "@everyone"}'
        embed.add_field(name="Open to:", value=role_info)
        msg = await channel.send(embed=embed)
        embed.set_footer(text=(f'Winners: {winners} | Ends at {fmt_end} UTC | Raffle ID: {msg.id}'))
